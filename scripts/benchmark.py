...
class StarcopMethaneDetector:
    ...
    def load_starcop_data(self, data_path: str, subset: str = 'test') -> Tuple[tf.data.Dataset, pd.DataFrame]:
        ...
        dataset = dataset.map(lambda x, y: (tf.py_function(
            load_hyperspectral_data, [x], tf.float32), y))
    ...
    def calculate_starcop_metrics(self, 
        ...
        if 'qplume' in metadata_df.columns:
            ...
            metrics.update({
                "f1_score_large_plumes": f1_score(
                    ground_truth[large_plumes], 
                    pred_binary[large_plumes]
                ),
                "f1_score_small_plumes": f1_score(
                    ground_truth[~large_plumes], 
                    pred_binary[~large_plumes]
                )
            })
    ...
    def run_benchmark(self, data_path: str, batch_size: int = 32) -> None:
        ...
        try:
            ...
            # Calculate metrics
            metrics = self.calculate_starcop_metrics(
                predictions, ground_truth, metadata_df)
